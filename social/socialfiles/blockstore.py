...
pageserver = dy_import_module("pageserver.repy", additional_globals=LIBREPY_EXPORTS)

# How long to sleep between checking for a new connection
CHECKINTERVAL= .010
...
def main(args):
  ...
  libfile.set_max_cached_blocks(512*4)

  # Setup the encryption keys
  got_keys = get_encryption_keys(args)
  if not got_keys:
    return
  log("Using keys:",KEYS[0][0],KEYS[1][0],'\n')
  ...
  if not is_setup:
    return
  log("Index Size:",DATA_FILES[0].size(),'\n')
  log("Data Size:",DATA_FILES[1].size(),'\n')

  # Update the file index
  FILE_INDEX.update(get_all_files())
  log("File Index:", FILE_INDEX,'\n')

  # Setup the HTTP page handlers
  setup_default_handlers()

  # Start the listeners
  stop_func = pageserver.setup_listener(myip, port,check_intv=CHECKINTERVAL)
  log("Listening on",myip+":"+str(port),"\n")
